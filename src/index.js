class Game {
  ...
  this.addPlayer = function(username, id) {
    players.push(new Player(username, 0, playerCount));
    this.playerCount++;
  }
  this.chooseHotseat = function() {
    this.hotseatPlayer = Math.floor(Math.random() * playerCount);
    this.hotseatPlayer = players[hotseatPlayer];
  }
  this.nextHotseat = function() {
    this.hotseatPlayer++;
    this.hotseatPlayer = players[hotseatPlayer];
  }
...
class QuestionPack {
  ...
  this.pickRandom = function(numQuestions) { // fixme im slow
    // assert n <= questions.length
    const shuffled = [...questions].sort(() => 0.5 - Math.random());
    return return shuffled.slice(0, numQuestions);
  }
...
berkeleyQuestions =
  [
    ...
    "What is the dumbest thing you've done in college?"
  ]
